...
export const CanvasTeamEvaluation = ({ teamData }: EvaluationProps) => {
  const { data: canvasQuestions, isLoading } = useGetEvaluationByIdQuery(EvaluationType.CANVAS) // id pitch = 2
  ...
  const handlePostEvaluation = async () => {
    if (!teamData || !canvasQuestions || canvasQuestions.length === 0) {
    ...
    const payload: Evaluation[] = Object.keys(values).map((idSubcriterio) => {
      ...
      const criterio = canvasQuestions?.find((criterio) =>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 text-[#3C14A4]">
      ...
      {showSuccess ? (
        ...
        <>
          {canvasQuestions?.map((criterio: CriterioAvaliacao) => (
